...
// console.log(getNames([{name:'lloyd', age: 32, shoeSize: 12}, {name:'jamie', age:21, shoeSize: 8}])).toStrictEqual(['dyoll', 'eimaj']);
...
const count = (target, input) => {
  // Solution code here...
  let amount= input.reduce((acc, val) => {
...
const totalSum = (input) => {
  // Solution code here...5+879431  `

  let sum = input.reduce((acc, val) => acc.concat(val)).reduce((acc, val) => acc + val);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let answer =input.map(value => value.filter(val => val % 5 === 0 && Number(val) === val).map(newValue => Math.pow(2, newValue)));
...
//   let correctAnswer= shortest[0].name;
...
let findShortest = (data) => {
  // Solution code here...
  let answer= data.reduce((acc, val) => {
    if (acc === {}) {
      acc = val;
    } else {
