...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  for(let i = 0; i <= 2; i++){
    if (checker(board[i][0], board[i][1], board[i][2])) return true;
    if (checker(board[0][i], board[1][i], board[2][i])) return true;
  }
  if (checker(board[0][0], board[1][1], board[2][2]) || checker(board[0][2], board[1][1], board[2][0])) return true;
  return false;
};

let checker = (val1, val2, val3) => {
